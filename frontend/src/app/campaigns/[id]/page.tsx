...
import { ArrowLeft, Play, Pause, CheckCircle, Users, Phone, TrendingUp, Clock, ExternalLink } from 'lucide-react';
    ...
    const load = async () => {
      try {
        const [cRes, sRes] = await Promise.all([
          ...
          campaignsApi.getSessions(params.id),
        ]);
      ...
      <div className="bg-[#1a1d24] rounded-xl border border-[#2a2d38]">
        <div className="px-5 py-4 border-b border-[#2a2d38] flex items-center justify-between">
          <h3 className="text-[#f1f5f9] font-semibold text-sm flex items-center gap-2">
            <Phone size={15} className="text-[#475569]" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#1e2028]">
              {sessions.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-10 text-center text-[#475569]">No calls yet</td>
                </tr>
              ) : sessions.map((s) => (
