...
import { formatCurrency, formatDate, formatDateTime, getDpdBgColor } from '@/lib/utils';
import type { Customer, CallSession } from '@/lib/types';

const MOCK_CUSTOMER: Customer = {
  id: 'cus-1', name: 'Rajesh Kumar', phone: '+91 9876543210',
  email: 'rajesh.kumar@email.com', loan_amount: 250000, outstanding_amount: 45000,
  dpd: 15, due_date: '2026-03-01', loan_id: 'LN2024001234',
  segment: 'standard', preferred_language: 'hi-IN', status: 'active',
  tags: ['urgent', 'follow-up'], created_at: new Date(Date.now() - 30 * 86400000).toISOString(),
};

const MOCK_SESSIONS: CallSession[] = Array.from({ length: 5 }, (_, i) => ({
  id: `sess-${i + 1}`, customer_id: 'cus-1',
  agent_type: i % 2 === 0 ? 'sarvam' : 'whisper_edge',
  flow_id: 'flow_standard', status: 'completed', tier: 'tier_2',
...
export default function CustomerDetailContent({ id }: { id: string }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await customersApi.update(id, form);
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5 pt-5 border-t border-[#2a2d38]">
          <div>
            ...
            <div className="flex items-center gap-1 text-[#f1f5f9] font-semibold">
              <IndianRupee size={13} />{formatCurrency(customer.loan_amount).replace('₹', '')}
            </div>
          </div>
          <div>
            <div className="text-[10px] text-[#475569] uppercase tracking-widest mb-1">Outstanding</div>
            <div className="flex items-center gap-1 text-red-400 font-semibold">
              <IndianRupee size={13} />{formatCurrency(customer.outstanding_amount).replace('₹', '')}
            </div>
          </div>
          <div>
        ...
        <div className="lg:col-span-1 bg-[#1a1d24] rounded-xl border border-[#2a2d38]">
          ...
          <div className="p-5 space-y-4">
            {[
              ...
              <div key={field}>
                <label className="block text-xs text-[#475569] mb-1.5">{label}</label>
                {editMode
                  ? <input type={type} className={inputClass} value={(form[field] as string) || ''} onChange={(e) => setForm({ ...form, [field]: e.target.value })} />
                  : <div className="text-[#94a3b8] text-sm">{(customer[field] as string) || '—'}</div>
                }
              </div>
            ))}
              ...
              <div className="flex flex-wrap gap-1.5 mb-2">
                {(form.tags || []).map((tag) => (
                  <span key={tag} className="flex items-center gap-1 bg-[#111318] border border-[#2a2d38] rounded-full px-2 py-0.5 text-xs text-[#94a3b8]">
                    {tag}
                    {editMode && <button onClick={() => removeTag(tag)} className="text-[#475569] hover:text-red-400 ml-0.5">×</button>}
                  </span>
                ))}
              </div>
              {editMode && (
                <div className="flex gap-2">
                  <input value={newTag} onChange={(e) => setNewTag(e.target.value)}
                    ...
                    placeholder="Add tag..." className="flex-1 bg-[#111318] border border-[#2a2d38] rounded-lg px-3 py-1.5 text-xs text-[#94a3b8] focus:outline-none focus:border-indigo-500/50" />
                  <button onClick={addTag} className="px-3 py-1.5 bg-indigo-600/20 border border-indigo-500/30 rounded-lg text-xs text-indigo-400 hover:bg-indigo-600/30 transition-colors">Add</button>
                </div>
              )}
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#1e2028]">
                {sessions.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-[#475569] text-sm">No call history</td></tr>
                ) : sessions.map((s) => (
