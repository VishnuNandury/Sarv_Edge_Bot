...
const FLOW_DEFINITIONS: Record<string, { nodes: Array<{ id: string; label: string; type: NodeType; description: string; x: number; y: number }>; edges: Array<{ id: string; source: string; target: string; label: string }> }> = {
  flow_basic: {
    nodes: [
      { id: 'start',       label: 'Intro',           type: 'start',    description: 'Greet & identify customer',    x: 150, y: 0   },
      { id: 'verify',      label: 'Verify Identity', type: 'action',   description: 'Confirm customer details',     x: 150, y: 130 },
      { id: 'inform',      label: 'Inform Amount',   type: 'action',   description: 'State outstanding balance',    x: 150, y: 260 },
      { id: 'ask_payment', label: 'Request Payment', type: 'decision', description: 'Ask for payment commitment',   x: 150, y: 390 },
      { id: 'committed',   label: 'Commitment',      type: 'action',   description: 'Record promise to pay',        x: 0,   y: 520 },
      { id: 'declined',    label: 'Declined',        type: 'action',   description: 'Note refusal reason',          x: 300, y: 520 },
      { id: 'end',         label: 'Close Call',      type: 'end',      description: 'Thank & disconnect',           x: 150, y: 650 },
    ],
    edges: [
  ...
  flow_standard: {
    nodes: [
      { id: 'start',     label: 'Intro',          type: 'start',    description: 'Greet & identify',           x: 200, y: 0   },
      { id: 'verify',    label: 'Verify',          type: 'action',   description: 'Confirm identity',           x: 200, y: 120 },
      { id: 'inform',    label: 'Inform Balance',  type: 'action',   description: 'Share due amount & date',    x: 200, y: 240 },
      { id: 'negotiate', label: 'Negotiate',       type: 'decision', description: 'Offer payment options',      x: 200, y: 360 },
      { id: 'full_pay',  label: 'Full Payment',    type: 'action',   description: 'Confirm full settlement',    x: 0,   y: 490 },
      { id: 'partial',   label: 'Part Payment',    type: 'dtmf',     description: 'Enter partial amount',       x: 200, y: 490 },
      { id: 'callback',  label: 'Callback',        type: 'action',   description: 'Schedule follow-up',         x: 400, y: 490 },
      { id: 'confirm',   label: 'Confirm Details', type: 'action',   description: 'Confirm payment plan',       x: 200, y: 620 },
      { id: 'end',       label: 'Close',           type: 'end',      description: 'End call with summary',      x: 200, y: 750 },
    ],
    edges: [
      { id: 'e1',  source: 'start',     target: 'verify',    label: '' },
      { id: 'e2',  source: 'verify',    target: 'inform',    label: '' },
  ...
  flow_field_visit: {
    nodes: [
      { id: 'greeting',       label: 'Greeting',       type: 'start',    description: 'Introduce call purpose',               x: 200, y: 0   },
      { id: 'visit_check',    label: 'Visit Check',    type: 'decision', description: 'Did employee make a field visit?',     x: 200, y: 130 },
      { id: 'visit_no_end',   label: 'No Visit',       type: 'end',      description: 'Customer says no visit was made',      x: 0,   y: 280 },
      { id: 'payment_check',  label: 'Payment Check',  type: 'decision', description: 'Was payment made during/after visit?', x: 400, y: 280 },
      { id: 'payment_no_end', label: 'No Payment',     type: 'end',      description: 'Customer confirms no payment made',    x: 250, y: 430 },
      { id: 'payment_amount', label: 'Payment Amount', type: 'action',   description: 'Capture how much was paid',            x: 550, y: 430 },
      { id: 'end',            label: 'Close Call',     type: 'end',      description: 'Confirm payment and close',            x: 550, y: 570 },
    ],
    edges: [
  ...
  flow_advanced: {
    nodes: [
      { id: 'start',       label: 'Intro',         type: 'start',    description: 'Greet & identify',              x: 250, y: 0   },
      { id: 'verify',      label: 'Verify',         type: 'action',   description: 'Multi-factor verify',           x: 250, y: 110 },
      { id: 'assess',      label: 'Assess Risk',    type: 'decision', description: 'Check DPD & segment',           x: 250, y: 220 },
      { id: 'low_risk',    label: 'Soft Ask',       type: 'action',   description: 'Gentle payment request',        x: 0,   y: 350 },
      { id: 'negotiate',   label: 'Negotiate',      type: 'decision', description: 'Offer EMI restructure',         x: 250, y: 350 },
      { id: 'high_risk',   label: 'Legal Notice',   type: 'action',   description: 'Mention legal action',          x: 500, y: 350 },
      { id: 'dtmf_amount', label: 'Enter Amount',   type: 'dtmf',     description: 'DTMF payment amount',           x: 50,  y: 480 },
      { id: 'waiver',      label: 'Offer Waiver',   type: 'action',   description: 'Partial interest waiver',       x: 250, y: 480 },
      { id: 'escalate',    label: 'Escalate',       type: 'action',   description: 'Transfer to supervisor',        x: 500, y: 480 },
      { id: 'commitment',  label: 'Commitment',     type: 'action',   description: 'Record PTP details',            x: 150, y: 610 },
      { id: 'end',         label: 'Close',          type: 'end',      description: 'Summary & close',               x: 250, y: 740 },
    ],
    edges: [
      { id: 'e1',  source: 'start',       target: 'verify',      label: '' },
